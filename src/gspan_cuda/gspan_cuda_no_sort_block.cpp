...
gspan_cuda_no_sort_block::gspan_cuda_no_sort_block()
{
  execute_tests = false;
  logger = Logger::get_logger("GSPAN_CUDA_NO_SORT_BLOCK");

  compute_support_max_memory_size = 1024 * 1024 * 50;
...
bool gspan_cuda_no_sort_block::should_filter_non_min(const types::DFS &dfs_elem, const types::DFSCode &code) const
{
  return true;
}

void gspan_cuda_no_sort_block::fill_supports(extension_element_t *d_exts_block,
  ...
  for(int i = 0; i < compute_support_calls; i++) {
    ...
    for(int dfs_id = 0; dfs_id < dfs_in_this_block; dfs_id++) {
      if(supports[dfs_id] >= minimal_support) {
        ...
        TRACE2(*logger, "dfs: " << h_block_dfs_elems[dfs_start_idx + dfs_id].to_string() << " is frequent, support: " << supports[dfs_id]);
      } else {
        TRACE2(*logger, "dfs: " << h_block_dfs_elems[dfs_start_idx + dfs_id].to_string() << " is NOT frequent, support: " << supports[dfs_id]);
      }
    } // for dfs_id
 ...
 *
 */
void gspan_cuda_no_sort_block::filter_extensions(extension_element_t *d_exts_result,
