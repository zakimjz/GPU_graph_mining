...
bool parse_int(const char *cstr, int &ret)
{
  ...
  //const char *cstr = str.c_str();
  uint_t cstr_len = strlen(cstr);
  int i = (int_t) strtol(cstr, &ptr, 10);
  // TODO: check space or trim str !!!
  if(ptr == cstr+cstr_len) {
    ret = i;
    return true;
...
bool parse_uint(const char *cstr, uint_t &ret)
{
  char *ptr = 0;
  //const char *cstr = str.c_str();
  uint_t cstr_len = strlen(cstr);
  uint i = (uint_t) strtoul(cstr, &ptr, 10);
  // TODO: check space or trim str !!!
  if(ptr == cstr+cstr_len) {
    ret = i;
    return true;
...
bool parse_long(const char *cstr, long_t &ret)
{
  char *ptr = 0;
  uint_t cstr_len = strlen(cstr);
  long_t i = strtol(cstr, &ptr, 10);
  if(ptr == cstr+cstr_len) {
    ret = i;
    return true;
  }
...
bool parse_ulong(const char *cstr, ulong_t &ret)
{
  char *ptr = 0;
  uint_t cstr_len = strlen(cstr);
  ulong_t i = strtol(cstr, &ptr, 10);
  if(ptr == cstr+cstr_len) {
    ret = i;
    return true;
  }
