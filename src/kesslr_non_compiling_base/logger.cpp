...
Logger *Logger::get_logger(string component, string format)
{
  if(global_file_buffer == 0) {
    Logger *tmp = get_logger(component, format, cout.rdbuf());
    //cout.tie(tmp);
    return tmp;
  } else {
...
Logger *Logger::get_logger(string component)
{
  ...
  if(global_file_buffer != 0) {
    ...
    Logger *tmp = get_logger(component, DEFAULT_LOG_FORMAT, cout.rdbuf());
    return tmp;
  }
}
...
{
  //cout << __FILE__ << "@" << __LINE__ << ": " << (void*)buf << endl << endl;
  //ios::rdbuf(buf);
  basic_ios<char>::rdbuf(buf);
  stream_buffer = buf;
}
...
{
  set_parallel_info(processor_rank, processor_number, true);

  std::stringstream ss;
  ss << filename_prefix.c_str() << processor_rank;
  Logger::set_global_logfile(ss.str(), true);
}

void Logger::set_global_streambuf(streambuf *buf, bool set_to_existing_loggers)
{
  if(set_to_existing_loggers) {
    for(map<string, Logger *>::iterator it = loggers->begin(); it != loggers->end(); it++) {
...
void Logger::set_global_logfile(const string &filename, bool set_to_existing_loggers)
{
  std::filebuf *fbuf = new std::filebuf();
  fbuf->open(filename.c_str(), std::ios::out | std::ios::trunc);

  set_global_streambuf(fbuf, set_to_existing_loggers);
