                                ...
                                uint *segment_sizes, uint *segment_count, uint num_segments)
{
	...
	while(range.x < range.y) {
        ...
        if(warp < (NUM_WARPS - 1) && lane == WARP_SIZE - 1) {
          ...
          //printf("tid: %d is last in the warp: %d; flag_after_this_block: %d\n", tid, warp, flag_after_this_block);
        ...
        */
        ...
        if(range.x + tid*VALUES_PER_THREAD < count && (flag_after_this_warp || flag_after_this_block || this_thread_has_flag || last_thread_last_value)) {
          int segment_index = 0;
          int block_values = 0;
          int dest_idx = 0;
